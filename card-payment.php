<?php
/**
 * ONE VISION COMMUNITY — VALIDATION DU PAIEMENT CARTE BANCAIRE (PAGE PENDING AUTOMATIQUE)
 */
  ...
  <main class="card-pending-container">
    <div class="card-pending-card">
      ...
      <div id="statusBadge" class="status-badge-pending">
        ...
        <span id="statusBadgeText">En cours de validation (PENDING)...</span>
      </div>

      <h1 id="pageTitle" style="font-size:1.6rem; font-weight:800; color:var(--color-text-main, #0f172a); margin-bottom:0.4rem; letter-spacing:-0.02em;">
        Traitement de votre paiement par carte
      </h1>
      <p id="pageSubtitle" style="font-size:0.92rem; color:var(--color-text-muted, #64748b); max-width:440px; margin:0 auto; line-height:1.5;">
        Votre demande de règlement de <strong><?= htmlspecialchars($orderAmount) ?></strong> a été déclenchée. Veuillez patienter pendant la confirmation auprès de votre banque.
      </p>

      <!-- Spinner animé pendant l'attente -->
      <div class="spinner-wrap" id="spinnerWrap">
        <div class="spinner-ring"></div>
        <span style="font-size:0.82rem; color:#4f46e5; font-weight:700; margin-top:0.75rem;" id="spinnerText">
          Vérification de l'autorisation bancaire 3D-Secure...
        </span>
      </div>
      ...
      <div class="card-meta-box">
        ...
        <div class="card-meta-row">
          ...
          <span class="card-meta-val" style="color:#16a34a; font-size:1rem;" id="metaAmount"><?= htmlspecialchars($orderAmount) ?></span>
        </div>
        <div class="card-meta-row">
          <span class="card-meta-label">Passerelle bancaire</span>
          <span class="card-meta-val" style="color:#2563eb;">SasaPay 3D-Secure 2.0</span>
        </div>
      </div>
      ...
      <div class="instructions-card-box" id="instructionsBox">
        ...
        <ol>
          <li>Votre banque examine l'autorisation de débit de <strong><?= htmlspecialchars($orderAmount) ?></strong>.</li>
          <li>Si une notification <strong>3D-Secure</strong> ou un code SMS apparaît sur votre téléphone, validez-le sur votre application bancaire.</li>
          <li><strong>Ne quittez pas cette page</strong> : la détection et la redirection vers votre espace membre sont entièrement automatiques.</li>
        </ol>
      </div>

      <!-- Sécurité & engagement -->
      <div style="margin-top:1.5rem; font-size:0.78rem; color:#94a3b8; display:flex; align-items:center; justify-content:center; gap:0.4rem;">
    ...
    document.addEventListener('DOMContentLoaded', () => {
      ...
      setTimeout(() => {
        ...
        if (statusBadge) {
          ...
          statusBadge.innerHTML = `<span class="pulse-dot-green"></span> <span>Paiement Validé avec Succès !</span>`;
        }
        if (spinnerWrap) spinnerWrap.style.display = 'none';
        if (checkIconWrap) checkIconWrap.classList.add('active');

        if (pageTitle) pageTitle.textContent = "Paiement Confirmé !";
        if (pageSubtitle) pageSubtitle.innerHTML = "Votre carte a été débitée avec succès. Redirection vers votre reçu et accès membre...";

        if (instructionsBox) {
          instructionsBox.style.background = '#dcfce7';
          instructionsBox.style.borderColor = '#86efac';
          instructionsBox.innerHTML = `
            <strong style="color:#166534; font-size:0.95rem;">✅ Authentification 3D-Secure Réussie</strong>
            <p style="margin:0; color:#15803d; font-size:0.86rem;">
              Votre banque a approuvé la transaction pour One Vision Community. Votre commande <strong>${orderNum}</strong> est désormais validée.
            </p>
          `;
        }
