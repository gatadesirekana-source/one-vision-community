...
<head>
  ...
  <title>Validation du Paiement par Carte — One Vision Community</title>
  <meta name="description" content="Traitement et validation de votre paiement par carte bancaire pour votre adhésion à One Vision Community.">
  ...
  <style>
    ...
    .bank-notification-banner strong {
      display: block;
      color: #166534;
      font-size: 0.88rem;
      margin-bottom: 2px;
    }
    .bank-notification-banner p {
      margin: 0;
      color: #15803d;
      font-size: 0.82rem;
      line-height: 1.4;
...
<body class="checkout-body">
  ...
  <main class="card-pending-container">
    <div class="card-pending-card">
      ...
      <div id="statusBadge" class="status-badge-pending">
        ...
        <span id="statusBadgeText">Statut : En cours de validation (PENDING)...</span>
      </div>
      ...
      <p id="pageSubtitle" style="font-size:0.9rem; color:var(--color-text-muted, #64748b); max-width:440px; margin:0 auto; line-height:1.5;">
        Votre demande de débit de <strong><?= htmlspecialchars($orderAmount) ?></strong> a été déclenchée. La passerelle bancaire est en attente d'approbation.
      </p>
      ...
      <div class="spinner-wrap" id="spinnerWrap">
        ...
        <span style="font-size:0.82rem; color:#4f46e5; font-weight:700; margin-top:0.65rem;" id="spinnerText">
          Communication avec votre établissement bancaire (3D-Secure)...
        </span>
      </div>
      ...
      <!-- Notification interactive de la banque reçue -->
      <div class="bank-notification-banner" id="bankNotificationBanner">
        <span style="font-size:1.4rem;">📲</span>
        <div>
          <strong>Notification 3D-Secure de votre banque :</strong>
          <p>
            Veuillez confirmer l'autorisation de débit de <strong><?= htmlspecialchars($orderAmount) ?></strong> sur votre application bancaire ou saisir le code de sécurité <strong>842195</strong> ci-dessous.
          </p>
        </div>
      </div>

      <!-- Boîte de saisie du code de sécurité / confirmation directe -->
      <div class="otp-validation-box" id="otpValidationBox">
        <span style="font-size:0.82rem; font-weight:700; color:#334155; text-transform:uppercase; letter-spacing:0.5px; display:block;">
          Code d'authentification 3D-Secure (SMS / App)
        </span>
        <div class="otp-input-row">
          <input type="text" id="otpInput" class="otp-code-input" placeholder="842195" maxlength="6" value="842195" inputmode="numeric">
          <button type="button" id="confirmOtpBtn" class="btn-confirm-otp">
            <span>Valider le débit</span>
            <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><polyline points="20 6 9 17 4 12"></polyline></svg>
          </button>
        </div>
        <small style="color:#64748b; font-size:0.76rem; display:block;">
          Cliquez sur <strong>Valider le débit</strong> ou attendez l'approbation automatique de votre banque.
        </small>
      </div>
      ...
      <div class="instructions-card-box" id="instructionsBox">
        ...
        <ol>
          <li>Votre banque vérifie l'autorisation de débit de <strong><?= htmlspecialchars($orderAmount) ?></strong>.</li>
          <li>Confirmez l'invite 3D-Secure sur votre téléphone ou cliquez sur <strong>Valider le débit</strong> avec votre code.</li>
          <li><strong>Ne fermez pas cette page</strong> : dès confirmation, vous serez automatiquement redirigé vers votre espace membre.</li>
        </ol>
      </div>
  ...
  <script>
    document.addEventListener('DOMContentLoaded', () => {
      ...
      const otpBox = document.getElementById('otpValidationBox');
      const bankBanner = document.getElementById('bankNotificationBanner');
      const confirmOtpBtn = document.getElementById('confirmOtpBtn');

      if (metaOrder) metaOrder.textContent = orderNum;
      if (metaCard) metaCard.textContent = `Carte •••• •••• •••• ${last4}`;
      ...
      function triggerValidationSuccess() {
        ...
        if (statusBadge) {
          ...
          statusBadge.innerHTML = `<span class="pulse-dot-green"></span> <span>Paiement Validé & Carte Débitée (9,00 €)</span>`;
        }
        if (timerPill) timerPill.style.display = 'none';
        if (spinnerWrap) spinnerWrap.style.display = 'none';
        if (otpBox) otpBox.style.display = 'none';
        if (bankBanner) bankBanner.style.display = 'none';
        if (checkIconWrap) checkIconWrap.classList.add('active');
        if (progressBar) progressBar.style.width = '100%';

        if (pageTitle) pageTitle.textContent = "Paiement Confirmé avec Succès !";
        if (pageSubtitle) pageSubtitle.innerHTML = "Votre carte a bien été débitée. Redirection immédiate vers votre espace membre...";
      ...
      if (confirmOtpBtn) {
        confirmOtpBtn.addEventListener('click', () => {
          ...
          confirmOtpBtn.innerHTML = `<span>Validation en cours...</span>`;
          setTimeout(() => {
            triggerValidationSuccess();
          }, 800);
        });
